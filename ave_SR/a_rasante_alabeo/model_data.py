...
eSize=0.25                             #tamaño medio de los elementos
...
# auxiliary sets
#   muro izquierdo
muri_z1_set=gridGeom.getSetSurfMultiRegion(lstIJKRange=[gm.IJKRange((0,pyhast1,0),(pxmnp1,pyhast1,lastZpos)),gm.IJKRange((pxmxp2,pyhast1,0),(lastXpos,pyhast1,lastZpos))],nameSet='muri_z1_set')
...
#   muro derecho
murd_z1_set=gridGeom.getSetSurfMultiRegion(lstIJKRange=[gm.IJKRange((0,pyhast2,0),(pxmnp1,pyhast2,lastZpos)),gm.IJKRange((pxmxp2,pyhast2,0),(lastXpos,pyhast2,lastZpos))],nameSet='murd_z1_set')
...
# Q1a1via: Tren de cargas posición 1 (en una sola vía)
Q1a1via=lcases.LoadCase(preprocessor=prep,name="Q1a1via",loadPType="default",timeSType="constant_ts")
Q1a1via.create()
Q1a1via.addLstLoads([Qptren_v1_e1,Qptren_v1_e2,Qptren_v1_e3,Qptren_v1_e4,
                 sc_qltren_carr1_a,sc_qltren_carr1_b,
                 sc_qltren_carr2_a,sc_qltren_carr2_b])
...
#Datos para cálculo de alabeo
xCarr1=xList[pxcarr1]
xCarr2=xList[pxcarr2]
xCarr3=xList[pxcarr3]
xCarr4=xList[pxcarr4]

nPoints=17

yControl=[-GH/2+i*0.5 for i in range(nPoints)]

zDintel=zList[lastZpos]
nudCarr1=[dintel.getNearestNode(geom.Pos3d(xCarr1,yControl[i],zDintel)) for i in range(nPoints)]
nudCarr2=[dintel.getNearestNode(geom.Pos3d(xCarr2,yControl[i],zDintel)) for i in range(nPoints)]
nudCarr3=[dintel.getNearestNode(geom.Pos3d(xCarr3,yControl[i],zDintel)) for i in range(nPoints)]
nudCarr4=[dintel.getNearestNode(geom.Pos3d(xCarr4,yControl[i],zDintel)) for i in range(nPoints)]

yNuds=[n.get3dCoo[1] for n in nudCarr1]
'''
for n in nudCarr4:
    print n.get3dCoo
'''
