...
Hrelli=4.24                   #ver gráfico en memoria de cálculo
Hrelld=4.32                   #ver gráfico en memoria de cálculo
Hbali=0.51                    #ver gráfico en memoria de cálculo
Hbald=0.57                    #ver gráfico en memoria de cálculo
Hcarr1=5.08                   #ver gráfico en memoria de cálculo
Hcarr2=5.16                   #ver gráfico en memoria de cálculo
Hcarr3=5.15                   #ver gráfico en memoria de cálculo
Hcarr4=5.24                   #ver gráfico en memoria de cálculo
...
Kbalasto=5.373e4*1e3                  #coef. balasto (N/m3)
...
xList=[0,daux1/3.,2*daux1/3.,
       9.18,9.22,11.43,12.38,13.56,15.26,16.78,18.85,20.54,21.74,23.08,24.91,
       24.95,24.95+daux2/3., 24.95+2*daux2/3.,
       35]
...
for j in range(len(yList)):
    r=gm.IJKRange((0,j,0),(lastXpos,j,lastZpos))
    gridGeom.movePointsRange(r,xc.Vector([lnSlope*yList[j],0,0]))
...
losas_M1M2=losCim_M1M2+dintel_M1M2
losas_M1M2.name='losas_M1M2'
losas_M1M2.description='Losas'

loscPlusHasti=losCim+hastIzq
loscPlusHasti.name='loscPlusHasti'
loscPlusHasti.description='Losa cim. + hastial'

angEsv=math.atan(4.81/9.4)  #angle (rad) of the shear reinforcement
                            #with respect to local axis 2 (parallel to
                            # global Y) of the horizontal elements
                            #(positive from X to Y)

for e in dintel.getElements:
    e.setProp("theta",angEsv)
for e in losCim.getElements:
    e.setProp("theta",angEsv)
...
#carga uniforme paso fauna + nivel agua
...
#del apoyo)
L=xList[-1]
pol=polygon(xCent=L/2.0,yCent=0,Lx=L+10,Ly=GH-deckTh)
...
pol=polygon(xCent=L/2.0,yCent=0,Lx=L+10,Ly=GH-baseSlabTh)
...
L=xList[-1]
pol=polygon(xCent=L/2.0,yCent=(baseSlabTh+GV)/2.0,Lx=L+10,Ly=GV-deckTh)
hastIzqCortante=sets.set_included_in_orthoPrism(preprocessor=prep,setInit=hastIzq_M1M2,prismBase=pol,prismAxis='Y',setName='hastIzqCortante')
