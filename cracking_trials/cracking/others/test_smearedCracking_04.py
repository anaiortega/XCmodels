...
A_s=5310e-6    #area of bottom reirnfocement layer (10 fi 26)
...
# report_material.report_concrete02(concrDiag=concrDiagram,paramTensStiffening=paramTS,grTitle='Ex. 7.5 EC2W. Concrete $\sigma-\epsilon$ curve',grFileName='ex7_5EC2W',texFileName='ex7_5EC2W.tex')


# stresses in a selected interval of the stress-strain graphic
epsMin=-0.7e-3
epsMax=-0.5e-3
nmbPoints=100
interval=(epsMax-epsMin)/nmbPoints
epsLst=[epsMin+i*interval for i in range(nmbPoints)]
sigmaLst=list()
for eps in epsLst:
...
if (abs(ratio1)<1e-5) & (abs(ratio2)<1e-5) & (abs(ratio4)<1e-3) & (abs(ratio5)<1e-2) & (abs(ratio6)<1e-3) & (abs(ratio7)<1e-2) & (abs(ratio8)<1e-3) & (abs(ratio9)<1e-3) & (abs(ratio10)<1e-5):
  print "test ",fname,": ok."
else:
