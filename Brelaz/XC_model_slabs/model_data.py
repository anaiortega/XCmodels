...
print 'modelDataScriptDir= ', modelDataScriptDir
...
gapSl=0.01*proj_fc   # gap between deck slabs
widthSl=(2.5-gapSl)*proj_fc # width slabs

firstSl=(0.8+2.5)*proj_fc-gapSl/2.0
...
sigmaPrestr=0.7*Fm_cable/area_cable      #prestressing stress [Pa]
prestrLoss=0.8                           #prestressing losses 
prestrFct=0.9                            #prestressing partial factor
...
deck_rg=[gm.IJKRange((0,i,0),(lastXpos,i+1,0)) for i in range(0,16,2) ]

curb_rg_a=[gm.IJKRange((0,i,0),(0,i+1,lastZpos)) for i in range(0,16,2)]
curb_rg_b=[gm.IJKRange((lastXpos,i,0),(lastXpos,i+1,lastZpos)) for i in range(0,16,2) ]
curb_rg=curb_rg_a+curb_rg_b
#Surfaces generation
...
cabl_mat=typical_materials.defCableMaterial(prep, name="cabl_mat",E=Ecabl,prestress=prestrFct*prestrLoss*sigmaPrestr,rho=0.0)
...
poly_lane_Lausanne.agregaVertice(geom.Pos2d(xList_deck[2],0))
...
poly_rest_Acc.agregaVertice(geom.Pos2d(xList_deck[lastXpos]-3,0))
...
combContainer.SLS.rare.add('ELSR02', '1.0*GselfWeight+1.0*GdeadLoad+1.0*QliveLoadB')

#Frequent combinations.
combContainer.SLS.freq.add('ELSF01', '1.0*GselfWeight+1.0*GdeadLoad+0.75*QliveLoadA')
combContainer.SLS.freq.add('ELSF02', '1.0*GselfWeight+1.0*GdeadLoad+0.75*QliveLoadB')
...
combContainer.ULS.perm.add('ELUB01', '1.35*GselfWeight+1.35*GdeadLoad+1.5*QliveLoadB')
combContainer.ULS.perm.add('ELUB02', '0.80*GselfWeight+0.80*GdeadLoad+1.5*QliveLoadB')
#accidental
