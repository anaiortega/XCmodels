...
#    lcs=GridModel.QuickGraphics(gridmodl)   #26/03/2017
    ...
    for st in self.setsToDispIntForc:
        for arg in self.listIntForc:
            ...
            grfname=pathGr+self.loadCaseName+st.name+arg
            lcs.displayIntForc(itemToDisp=arg,setToDisplay=st,viewDef=self.cameraParameters,fileName=grfname+'.jpg')
            lcs.displayIntForc(itemToDisp=arg,setToDisplay=st,viewDef=self.cameraParameters,fileName=grfname+'.eps')
            capt=self.loadCaseDescr + '. ' + st.name.capitalize() + ', ' + capStdTexts[arg] + ' ' + unDesc
            insertGrInTex(texFile=texFile,grFileNm=grfname,grWdt=grWdt,capText=capt)
    ...
    for st in self.setsToDispBeamIntForc:
        for arg in self.listBeamIntForc:
            ...
            grfname=pathGr+self.loadCaseName+st.name+arg
            lcs.displayIntForcDiag(itemToDisp=arg,setToDisplay=st,viewDef=self.cameraParametersBeams,fileName=grfname+'.jpg')
            lcs.displayIntForcDiag(itemToDisp=arg,setToDisplay=st,viewDef=self.cameraParametersBeams,fileName=grfname+'.eps')
            capt=self.loadCaseDescr + '. ' + st.name.capitalize() + ', ' + capStdTexts[arg] + ' ' + unDesc
            insertGrInTex(texFile=texFile,grFileNm=grfname,grWdt=grWdt,capText=capt)
