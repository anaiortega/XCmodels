...
stemBottomWidth= 8*INCH_2_METER
stemTopWidth= stemBottomWidth
footingThickness= 14*INCH_2_METER
eccentricity= stemTopWidth/4.0
sectionName= "RWpinned_west"
...
wall.stemReinforcement.setReinforcement(1,A13_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.stemReinforcement.setReinforcement(2,A13_15.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.footingReinforcement.setReinforcement(3,A13_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.stemReinforcement.setReinforcement(4,A10_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.stemReinforcement.setReinforcement(5,A16_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.stemReinforcement.setReinforcement(6,A13_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.footingReinforcement.setReinforcement(7,A10_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.footingReinforcement.setReinforcement(8,A16_30.getCopy(ACI_limit_state_checking.RebarController('B')))
wall.stemReinforcement.setReinforcement(11,A13_30.getCopy(ACI_limit_state_checking.RebarController('B')))
...
buildingLoadFactor= 1.0
buildingDeadLoad= buildingLoadFactor*56.62e3 # Ramp cover + second floor trusses + first floor planks
print('buildingDeadLoad= ', buildingDeadLoad/1e3, ' kN/m')
wall.createLoadOnTopOfStem(xc.Vector([0.0,-buildingDeadLoad,-buildingDeadLoad*eccentricity]))

#Live load. Traffic load.
trafficLoad= loadCaseManager.setCurrentLoadCase('trafficLoad')
trafficEarthPressure= earth_pressure.StripLoadOnBackfill(qLoad= 11.97e3,zLoad= 0.0, distWall= 0.5, stripWidth= 10)
...
buildingLiveLoad= buildingLoadFactor*33.03e3 # Ramp cover + second floor trusses + first floor planks
print('buildingLiveLoad= ', buildingLiveLoad/1e3, ' kN/m')
wall.createLoadOnTopOfStem(xc.Vector([0.0,-buildingLiveLoad,-buildingLiveLoad*eccentricity]))

# Snow load from the building.
snowLoad= loadCaseManager.setCurrentLoadCase('snowLoad')
buildingSnowLoad= buildingLoadFactor*13.38e3 
print('buildingSnowLoad= ', buildingSnowLoad/1e3, ' kN/m')
wall.createLoadOnTopOfStem(xc.Vector([0.0,-buildingSnowLoad,-buildingSnowLoad*eccentricity]))

# Wind load from the building.
windLoad= loadCaseManager.setCurrentLoadCase('windLoad')
buildingWindLoad= buildingLoadFactor*-8.46e3 
print('buildingWindLoad= ', buildingWindLoad/1e3, ' kN/m')
wall.createLoadOnTopOfStem(xc.Vector([0.0,-buildingWindLoad,-buildingWindLoad*eccentricity]))
