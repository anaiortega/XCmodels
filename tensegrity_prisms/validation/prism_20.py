...
#Data
#    geometry
nSidPol=20     # number of sides of the regular n-polygon
...
print '20-plex &',round(t_strut,4),' & ',round(l_strut,4),' & ',round(q_strut,4),' & ',round(t_diag,4),' & ',round(l_diag,4),' & ',round(q_diag),' & ',round(t_sadd,4),' & ',round(l_sadd,4),' & ',round(q_sadd,4),' & ',round(q_strut+q_diag,6),' & ',round(q_strut+2*math.sin(math.pi/(1.0*nSidPol))*q_sadd,6), '\\\\'
...
lcs.displayIntForcDiag(itemToDisp='N',setToDisplay=xcTotalSet.elSet,fConvUnits= 1.0e-3,scaleFactor=1,unitDescription=': Axial internal forces [kN] ', viewDef= vtk_graphic_base.CameraParameters('XYZPos',1),fileName='plex_20.jpg',defFScale=40.0)
quit()
